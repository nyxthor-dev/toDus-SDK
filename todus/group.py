...
from .errors import AuthenticationError, GroupError
...
class GroupClient:
    ...
    def send_event(self, group_id: str, title: str, start: int, end: int,
        ...
        msg = stanza.group_event_message(
            group_jid,
            event_id,
            title,
            start,
    ...
    def delete_message(
        ...
        msg = stanza.group_delete_message(
            group_jid,
            message_id,
            mid,
            body=body,
    ...
    def create_group(self, room_name: str, members: list[str] = None) -> str:
        ...
        Args:
            room_name: nombre del grupo.
            members: lista de teléfonos a invitar (opcional).

        Returns:
            El msg_id de la petición. La confirmación llega por listen_messages().
        """
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        mid = self._generate_msg_id()
        iq_msg = stanza.group_create_iq(room_name, occupants=members, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    def get_my_groups(self, index: int = 0, limit: int = 50) -> str:
        """Lista mis grupos de forma paginada (todus:muclight:my_mucs:2).

        Returns:
            El msg_id de la petición. El resultado llega por listen_messages().
        """
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        mid = self._generate_msg_id()
        iq_msg = stanza.group_my_groups_iq(index=index, limit=limit, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    def promote_admin(self, group_id: str, phones: list[str]) -> str:
        """Promueve miembros a administradores (td:g:promote)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        group_jid = self._get_group_jid(group_id)
        mid = self._generate_msg_id()
        iq_msg = stanza.group_promote_admin_iq(group_jid, phones, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    def demote_admin(self, group_id: str, phones: list[str]) -> str:
        """Degrada administradores a participantes (td:g:demote)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        group_jid = self._get_group_jid(group_id)
        mid = self._generate_msg_id()
        iq_msg = stanza.group_demote_admin_iq(group_jid, phones, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    def get_group_info_by_link(self, link: str) -> str:
        """Obtiene info de un grupo por su enlace de invitación (td:g:info_by_link)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        mid = self._generate_msg_id()
        iq_msg = stanza.group_info_by_link_iq(link, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    def get_group_info_by_id(self, group_id: str) -> str:
        """Obtiene info de un grupo por su id (td:g:info_by_id)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        mid = self._generate_msg_id()
        iq_msg = stanza.group_info_by_id_iq(group_id, msg_id=mid)
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(iq_msg.encode())
        return mid

    # --- Envíos de nuevos tipos de mensaje a grupos ---

    def _send_group_msg(self, group_id: str, stanza_str: str, mid: str) -> str:
        with self.client._xmpp_session(self.client.token) as sock:
            sock.sendall(stanza_str.encode())
        return mid

    def send_voice(self, group_id: str, url: str, file_name: str,
                   file_size: int, duration: int, wave_sample: str = "",
                   caption: str = "", reply_to_id: str = "") -> str:
        """Envía nota de voz a un grupo (voice:n)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        self.client._rate_limiter.wait()
        mid = self._generate_msg_id()
        msg = stanza.voice_message(self._get_group_jid(group_id), url, file_name,
                                   file_size, duration, wave_sample,
                                   msg_id=mid, msg_type="gc", caption=caption,
                                   reply_to_id=reply_to_id)
        return self._send_group_msg(group_id, msg, mid)

    def send_gif(self, group_id: str, url: str, file_name: str,
                 file_size: int, width: int = 0, height: int = 0,
                 thumbnail: str = "", caption: str = "", reply_to_id: str = "") -> str:
        """Envía GIF a un grupo (gif:n)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        self.client._rate_limiter.wait()
        mid = self._generate_msg_id()
        msg = stanza.gif_message(self._get_group_jid(group_id), url, file_name,
                                 file_size, width, height, thumbnail, caption,
                                 msg_id=mid, msg_type="gc", reply_to_id=reply_to_id)
        return self._send_group_msg(group_id, msg, mid)

    def send_reaction(self, group_id: str, reacted_msg_id: str, reaction_code: str) -> str:
        """Envía una reacción a un mensaje del grupo (reaction:n)."""
        if not self.client.token:
            raise AuthenticationError("No autenticado")
        self.client._rate_limiter.wait()
        mid = self._generate_msg_id()
        msg = stanza.reaction_message(self._get_group_jid(group_id), reacted_msg_id,
                                      reaction_code, msg_id=mid, msg_type="gc")
        return self._send_group_msg(group_id, msg, mid)
