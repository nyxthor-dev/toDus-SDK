...
def create_channel_iq(
    ...
    attrs = []
    if name:
        attrs.append(f"name='{util.escape_xml(name)}'")
    if picture:
        attrs.append(f"profile_photo='{util.escape_xml(picture)}'")
    if link:
        attrs.append(f"link='{util.escape_xml(link)}'")
    attrs.append(f"public='{public}'")
    if subs:
        attrs.append(f"subs='{util.escape_xml(','.join(subs))}'")
    if desc:
        attrs.append(f"desc='{util.escape_xml(desc)}'")
...
def subscribe_channel_iq(channel_jid: str, msg_id: str = "") -> str:
    ...
    payload = "<query xmlns='todus:ch:subscribe'/>"
    return iq(type_="set", iq_id=mid, payload=payload, to=channel_jid)
...
def leave_channel_iq(channel_jid: str, msg_id: str = "") -> str:
    ...
    payload = "<query xmlns='todus:ch:leave'/>"
    return iq(type_="set", iq_id=mid, payload=payload, to=channel_jid)
...
def get_channel_publications_iq(channel_jid: str, last_id: str = "", limit: int = 25, msg_id: str = "") -> str:
    ...
    attrs = []
    if last_id:
        attrs.append(f"last_id='{util.escape_xml(last_id)}'")
    if limit:
        attrs.append(f"limit='{limit}'")
...
def get_my_channels_iq(msg_id: str = "") -> str:
    ...
    payload = "<query xmlns='todus:ch:my_channels:2'/>"
